...
using AdPlatformStorage.Server.Settings;

namespace AdPlatformStorage.Server.Deployment
{
    ...
    public class Startup
    {
            ...
            Action<HostBuilderContext, IServiceCollection> additionalConfigureServices = null)
        {
            ...
            return new HostBuilder()
                    ...
                    webHostBuilder
                        .ConfigureAppConfiguration((context, configurationBuilder) =>
                        {
                            ...
                            foreach (var enabledModuleInfo in _modules.Where(module => module.Value.Enabled))
                            {
                                Log.Logger?.Information(
                                    "Adding module \"{moduleKey}\" from assembly \"{moduleAssembly}\"",
                            ...
                            var webHostConfigurators = AppDomain.CurrentDomain.GetAssemblies()
                                ...
                                .Where(assemblyDefinedType =>
                                    assemblyDefinedType.ImplementsServiceType<IWebHostConfigurator>())
                                .Select(webHostConfiguratorType =>
                                    (IWebHostConfigurator)Activator.CreateInstance(webHostConfiguratorType)!)
                                .ToList();
                        ...
                        .Configure((_, applicationBuilder) =>
                        {
                            var applicationConfigurators = AppDomain.CurrentDomain.GetAssemblies()
                                ...
                                .SelectMany(assembly => assembly.DefinedTypes)
                                .Where(assemblyDefinedType =>
                                    assemblyDefinedType.ImplementsServiceType<IApplicationConfigurator>())
                                .Select(applicationConfiguratorType =>
                                    (IApplicationConfigurator)Activator.CreateInstance(applicationConfiguratorType)!)
                                .ToList();
                            ...
                            if (applicationConfigurators.Count > 1)
                            {
                                throw new InvalidOperationException(
                                    "Only one IApplicationConfigurator instance is supported");
                            }
                ...
                .ConfigureHostConfiguration(hostBuilder => { hostBuilder.AddEnvironmentVariables(); })
